...
SELECT AVG(ctscore.[TotalPrice]) AS [Avg Score TotalPrice], 
       AVG(COALESCE(ctmodel.[Average TotalPrice 2014 F], @FallBack2014Avg)) AS [Avg Model TotalPrice]
FROM CTE_ScoreSet ctscore
LEFT JOIN CTE_ModelSet ctmodel ON (ctscore.[Channel] = ctmodel.[Channel] AND ctscore.[PaymentType] = ctmodel.[PaymentType]);
GO
			...
			(
				...
				FROM (
						...
						LEFT JOIN CTE_ModelSet ctmodel 
							ON (ctscore.[Channel] = ctmodel.[Channel] AND ctscore.[PaymentType] = ctmodel.[PaymentType])
					) a
			) b 
GROUP BY b.[Decile]
ORDER BY b.[Decile];
